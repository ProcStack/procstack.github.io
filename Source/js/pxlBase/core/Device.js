...
export class Device{
    ...
    this.projectTitle=projectTitle;
		...
		this.x=sW*.5;
		this.y=sH*.5;
		this.screenRes=new THREE.Vector2( 1/sW,1/sH );
		this.screenAspect=new THREE.Vector2( 1,1);//sH/sW,sW/sH );
		this.screenRatio=new THREE.Vector2( sW/sH, sH/sW );
		this.origRes=new THREE.Vector2( sW, sH );
		this.screenResDeltaPerc=new THREE.Vector2( 1, 1 );
		this.mapW=1;
		this.mapH=1;
		//this.wheelDelta=0;
		
    this.gammaCorrection = new THREE.Vector3(1,1,1);
    ...
    //      Controller Module?
      ...
      'lock':false,
			...
			'velocityEase':new THREE.Vector2(0,0),
			'velocityEasePrev':null, // new THREE.Vector2(0,0)
			'velocity':new THREE.Vector2(0,0), //vec2
			'mBlurVelInf':new THREE.Vector2(2*this.screenRes.x,2*this.screenRes.y),
			'prevDeltaPos':[0,0,0], //vec2
			'endPos':null, //  [x,y] 
			'latchMatrix':null, // Mat4
			'netDistance':new THREE.Vector2(0,0), //vec2
			'netDistYPerc':0, //vec2
			'curDistance':new THREE.Vector2(0,0), //vec2
			'curFadeOut':new THREE.Vector2(0,0), //vec2
			'curStepDistance':new THREE.Vector2(0,0), //vec2
  ...
  setGammaCorrection( value=null ){
    if( value != null ){ // Set from Settings Menu
      this.gammaCorrection = new THREE.Vector3( 1/value, value, value );
      return;
    }
    ...
    this.gammaCorrection = new THREE.Vector3( 1/toGamma, shadowShift, shadowBoost );
  }
  ...
  // -- -- -- -- -- -- -- -- -- -- //
	...
	getMouseXY(e){
		...
		if(!this.mobile){
			...
			try{
				...
				//pxlDevice.touchMouseData.startPos=new THREE.Vector2(this.mouseX,this.mouseY);
				//pxlDevice.touchMouseData.endPos=new THREE.Vector2(this.mouseX,this.mouseY);
			}catch(err){};
		}
	...
	mapLockCursor(lock=false, button){
		if(this.canCursorLock && !this.mobile ){
			...
			if(lock==true){
                this.pxlGuiDraws.mapCanvas.focus();
				this.pxlGuiDraws.mapCanvas.requestPointerLock();
			}else{
	...
	mapOnDown(e){
		...
		if( target.getAttribute && target.getAttribute("id") == this.mapCore && this.pxlTimer.active ){
			...
			this.touchMouseData.startPos=new THREE.Vector2(this.mouseX,this.mouseY);
			this.touchMouseData.endPos=new THREE.Vector2(this.mouseX,this.mouseY);
			this.touchMouseData.curDistance=new THREE.Vector2(0,0);
			this.touchMouseData.curStepDistance=new THREE.Vector2(0,0);
	...
	mapOnMove(e){
		...
		if( (target.getAttribute && target.getAttribute("id") == this.mapCore && this.pxlTimer.active) || this.cursorLockActive){
	...
	mapOnUp(e){
        ...
        if( target.getAttribute && target.getAttribute("id") == this.mapCore){
	...
	endTouch(e) {
        ...
        if( this.touchMouseData.dragCount < 10 && target.getAttribute && target.getAttribute("id")==this.mapCore ){
	...
	async keyUpCall(e){
		...
		if(document.activeElement.type==undefined ){
          ...
          let dlData=this.pxlGuiDraws.mapCanvas.toDataURL("image/png");
	...
	resizeRenderResolution( iWidth=null, iHeight=null ){
		...
		if(this.pxlEnv.geoList['HDRView']){
			...
			//let rV=this.mapW>this.mapH ? this.mapH/this.mapW : 1;
		...
		this.touchMouseData.mBlurVelInf=new THREE.Vector2(2*this.screenRes.x,2*this.screenRes.y);
        ...
        // this.pxlEnv.mapOverlayHeavyPass.uniforms.ratio.value = this.mapW>this.mapH ? this.mapH/this.mapW : this.mapW/this.mapH;
		// this.pxlEnv.mapOverlayPass.uniforms.ratio.value = this.mapW>this.mapH ? this.mapH/this.mapW : this.mapW/this.mapH;
		// this.pxlEnv.mapOverlaySlimPass.uniforms.ratio.value = this.mapW>this.mapH ? this.mapH/this.mapW : this.mapW/this.mapH;
		...
		this.pxlGuiDraws.mapCanvas.width=this.sW;
		this.pxlGuiDraws.mapCanvas.height=this.sH;
