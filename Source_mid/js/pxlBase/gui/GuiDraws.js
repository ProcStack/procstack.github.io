...
export class GuiDraws{
    ...
    let inner;
    if(this.pxlQuality.detailLimit==0){
...
///////////////////////////////////////////
    ...
    let hudObjDisplay = false
      ...
      hudObjDisplay="inline-block";
			...
			if( !this.checkOpenWindows( close ) ){ // Returns if any windows are open, if not, hide the background object.
				hudObjDisplay="none";
				if(this.mapCanvas){ this.mapCanvas.focus(); }
			}
		}
	...
	*/
    ...
    checkFocus( targetId=null, isMapCore=null ){
        if( isMapCore===true ){
	...
	buildTopBar(){
    if( this.pxlDevice.mobile || !this.buildGuiStatus.userTopBar ){
        return;
    }
    ...
    //%=
	statsIconPrep( run=0 ){
		let curSVG=this.hudIcons.statsIcon.svg;
		if(curSVG){ // If this doesn't pass, means loading the svg broke
			
			let min=curSVG.getElementById("stats_min");
			let mid=curSVG.getElementById("stats_mid");
			let max=curSVG.getElementById("stats_max");
			if(min && mid && max){
				this.hudIcons.statsIcon.state=true;
				
				this.hudIcons.statsIcon.onAnim=curSVG.getElementById("stats_min_on");
				this.hudIcons.statsIcon.offAnim=curSVG.getElementById("stats_max_off");
				
				this.jmaJoinState( false );
			}else{
				if(run==2){
	...
	//%
	jmaJoinState( state=false ){
		if(this.hudIcons.statsIcon){
			if(this.hudIcons.statsIcon.onAnim && this.hudIcons.statsIcon.state!=state){
				if(state){
					this.hudIcons.statsIcon.onAnim.beginElement();
				}else{
					this.hudIcons.statsIcon.offAnim.beginElement();
				}
				this.hudIcons.statsIcon.state=state;
			}
		}
	}
      ...
      return;
		...
		html+=`
			<div id="hud_chatIconBlock" class="hud_bottomLeftBlockStyle">
                ...
                `;
        if( !this.pxlDevice.mobile && this.buildGuiStatus.verseList ){
...
///////////////////////////////////////////
    ...
    addSlider(parentObj, label, val, min, max, step, callback, args){
        if( typeof(parentObj) == "string" ){
            parentObj=document.getElementById("guiShaderUserSliders");
        }
    ...
    updateShaderTextFields( objRead=null ){
        ...
        let type="shaderGui";
        if( !objRead ){
            objRead= this.pxlEnv.currentRoom==this.pxlEnv.mainRoom ? this.guiWindows[type].currentShader : this.pxlEnv.roomSceneList[ this.pxlEnv.currentRoom ].getCurrentShader();
        }
		try{
    ...
    buildShaderEditor(){
      ...
      this.prepPromptFader( shaderDiv );
      this.guiWindows[type]={};
      this.guiWindows[type].gui=shaderDiv;
      this.guiWindows[type].active=false;
      
          
          let geoKeys=Object.keys( this.pxlEnv.geoList );
          let objectShaderOptions="";
          geoKeys.forEach( (k)=>{
              if( ["Mesh","Points"].includes( this.pxlEnv.geoList[k].type ) ){
                  if( this.pxlEnv.geoList[k].material.type == "ShaderMaterial" ){
                      let curOpt=k.substr(0,1).toUpperCase()+k.substring(1,k.length);
      ...
      this.guiWindows[type].shaderSliders=shaderSliders;
      ...
      this.guiWindows[type].updateObj=document.getElementById("shader_updateShader");
      this.guiWindows[type].headSpacer=document.getElementById("gui_shaderHeaderSpacer");
      this.guiWindows[type].shaderList=document.getElementById("gui_shaderEditorHeaderList");
      this.guiWindows[type].shaderFieldParent=document.getElementById("guiShaderFieldParent");
      ...
      this.guiWindows[type].shaderSelect=shaderPicker;
      ...
      this.guiWindows[type].updateObj.addEventListener("click", ()=>{
        ...
        vertObj.blur();
        fragObj.blur();
        tmpThis.mapCanvas.focus();
      });
          
      this.guiWindows[type].vertObj.onfocus=(e)=>{ tmpThis.focusShaderMessage(e,'vertObj'); };
      this.guiWindows[type].vertObj.onkeydown=(e)=>{ tmpThis.keyShaderMessage(e); };
      this.guiWindows[type].vertObj.onmousedown=(e)=>{ tmpThis.mDownShaderMessage(e); };
      this.guiWindows[type].vertObj.onclick=(e)=>{ tmpThis.clickShaderMessage(e); };
      this.guiWindows[type].vertObj.ondblclick=(e)=>{ tmpThis.dblclickShaderMessage(e); };
      this.guiWindows[type].fragObj.onfocus=(e)=>{ tmpThis.focusShaderMessage(e,'fragObj'); };
      this.guiWindows[type].fragObj.onkeydown=(e)=>{ tmpThis.keyShaderMessage(e); };
      this.guiWindows[type].fragObj.onmousedown=(e)=>{ tmpThis.mDownShaderMessage(e); };
      this.guiWindows[type].fragObj.onclick=(e)=>{ tmpThis.clickShaderMessage(e); };
      this.guiWindows[type].fragObj.ondblclick=(e)=>{ tmpThis.dblclickShaderMessage(e); };
      this.guiWindows[type].mouseX=0;
      this.guiWindows[type].mouseY=0;
      this.guiWindows[type].prevSelectStart=0;
      this.guiWindows[type].prevSelectEnd=0;
    }
    mDownShaderMessage(e){
        this.guiWindows["shaderGui"].mouseX=e.x;
        this.guiWindows["shaderGui"].mouseY=e.y;
    }
    clickShaderMessage(e){
        if( !this.guiWindows["shaderGui"].dcActive ){
            // ## Not sure why this was needed, but must have been a patch for some jankie antibody gui works
            let dx=Math.abs( this.guiWindows["shaderGui"].mouseX-e.x );
            let dy=Math.abs( this.guiWindows["shaderGui"].mouseY-e.y );
            ...
            if(desel){
                ...
                if(posStart!=posEnd){
                    this.guiWindows["shaderGui"].prevSelectStart=posStart;
                    this.guiWindows["shaderGui"].prevSelectEnd=posEnd;
                }
            }
        }
    }
    dblclickShaderMessage(e){
        let start=this.guiWindows["shaderGui"].prevSelectStart;
        let end=this.guiWindows["shaderGui"].prevSelectEnd;
        let textObj=e.target;
        ...
        this.guiWindows["shaderGui"].dcActive=true;
        setTimeout( ()=>{
            this.guiWindows["shaderGui"].dcActive=false;
        },500);
    }
	keyShaderMessage(e){
		if( e.repeat){ return; }
        ...
        if( keyHit=="Enter" && ctrl){
            this.guiWindows["shaderGui"].updateObj.click();
            textObj.focus();
    ...
    if(this.hudBottomBar){
      bBarHeight = this.hudBottomBar.offsetHeight
        ...
        this.guiWindows[type].fragObj.displayHeight=pHeight*.6;
        this.guiWindows[type].fieldBodyHeight=pHeight;
			}
		});
	}
