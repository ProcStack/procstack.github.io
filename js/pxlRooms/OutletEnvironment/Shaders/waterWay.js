...
// -- -- --
  ...
  void main(){
    ...
    float coastInfInv = max(0.0, 1.0-coastInf*(coastInf*.5+.5)); 
    ...
    Cd.rgb *= mix(  max(nCd.g*(nCd.r*.5+(0.85-coastInf))*.7*coastInfInv+.3,nCd.b*alpha)*.9*coastInfInv+.1,
                    rippleInf*.35+0.75,
                    min(1.0,(nCd.g*nCd.b)*coastInfInv+coastInf+rippleInf*.35 + (1.0-angleIncidence) ) 
                  ); 
