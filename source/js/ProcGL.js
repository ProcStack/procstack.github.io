// Initially developed for a multiplayer sytem,
//   Its been stripped down for ProcStack.github.io
//     So if you see remnants, pointless functions, other... junk, you know why
//
// TODO :
...
class ProcGL{
  constructor(NeurousCore, target, source){
  ...
  init(){
    ...
    if( !isWebGL2 ){
      ...
      console.log( "Or you are being quite catious, to each their own!" );
    }
    ...
    this.shaderPasses.blurYShaderPass.material.transparent = true
		...
		this.shaderPasses.scatterMixShaderPass = new ShaderPass(
			new THREE.ShaderMaterial( {
				...
				vertexShader: Shaders.Defaults.vert(),
				fragmentShader: Shaders.ScatterShaderPass.scatterMixShaderPass(),
				defines: {}
			} ), "tDiffuse"
		);
  ...
  buildPoints(){
    ...
    if( this.NeurousCore && this.NeurousCore.hasOwnProperty("emitters") && this.NeurousCore.emitters.hasOwnProperty("pointTrails") ){
      ...
      this.NeurousCore.emitters.pointTrails.forEach( (e)=>{
		...
		this.debug && console.log(`Generated vertex count - ${vertexCount}`);
    ...
    //const atlasGen=()=>{ return Math.floor( (Math.random() * 4000) % atlastGridSize ) * atlastSizeDiv; }
		...
		// TODO : Can this be updated to 16bit addresses?
  ...
  buildAttrFeedbackPoints(){
    ...
    this.debug && console.log("build shader program attempt")
  ...
  render( time ){
    this.time.x = time
    this.pointTexture.needsUpdate = true; // HTML5 Canvas Draw Texture
