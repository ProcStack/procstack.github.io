import*as l from"../../../Source/js/libs/three/three.module.js";function g(){return`
    #ifdef GL_FRAGMENT_PRECISION_HIGH
      precision highp float;
    #else
      precision mediump float;
    #endif
        `}import*as S from"../../../Source/js/libs/three/three.module.js";function A(){let a=g();return a+=`
  ...
  void main(){
    ...
    void main() {
      ...
      gl_FragColor = outCd;
    }`,a}import*as b from"../../three.module.js";var w=b.Vector2,T=b.Vector3;var ie={NONE:0,ERROR:1,WARN:2,INFO:3},re={OFF:0,LOW:1,MEDIUM:2,HIGH:3},le={verbose:ie.NONE,pxlRoomRoot:"./pxlRooms",staticCamera:!1,autoCamera:!1,antiAliasing:re.LOW,LoadEnvAssetFile:!1};import*as p from"../../../Source/js/libs/three/three.module.js";import*as P from"../../../Source/js/libs/three/three.module.js";function Y(a=!1){let e=`
    attribute vec3 color;
    ...
    void main(){
        ...
        `,a&&(e+=`
          ...
          `),e+=`
    }`,e}function X(a,e,i,t,s,o){let r=!1,v=1;a.hasOwnProperty("Specular")&&a.Specular>0&&(r=!0,v=a.Specular);let d=!1;a.PointColor&&(d=!0);let h=!0,u="1.0";a.hasOwnProperty("SurfaceNoise")&&(a.SurfaceNoise%1==0&&(u=a.SurfaceNoise+".0"),u=="0.0"&&(h=!1));let n=`
    ...
    `,t&&(n+=`
      ...
      `),n+=`
    void main(){
      `,d)n+="vec3 vertCd = vCd;";else if(e){let m=F=>F%1==0?F+".0":F+"",U=m(e.r),ee=m(e.g),te=m(e.b);n+=`vec3 vertCd = vec3( ${U}, ${ee}, ${te} ) ;`}else n+="vec3 vertCd = texture2D(dTexture,vUv).rgb ;";n+=`
        ...
        `,i&&(n+=`
          ...
          `);let y="",f="",c="";if(h&&(u!="1.0"&&(y="*"+u),n+=`
            float detailMult = (texture2D(detailTexture,vUv*4.0).r)*.5+.5;
            `,f=`+detailMult * (shadowInf*.5+.5) ${y}`,c=`(detailMult+.5) ${y}`),t&&(n+=`
  ...
  `,a>0&&(i+=`uniform vec3[${a}] lightPos;`),i+=`
  ...
  `,a>1?i+=`
    ...
    void main(){
        ...
        gl_FragColor=Cd;
    }`,a}var E=class{constructor(e=null,i="particles"){this.name=i,this.room=e,this.geometry=null,this.material=null,this.points=null,this.count=-1,this.pscale=new w(0,0),this.position=new T(0,0,0),this.atlasPath="sprite_dustLiquid.png",this.atlasPath="sprite_dustAtlas.png"}build(e=30,i=6,t=4,s=null){s||(s=this.elementDuplicator([[0,.75],[0,.5],[.25,.75],[.25,.5]],4)),this.addToScene(e,i,t,s)}setPosition(...e){e.length===1?this.position=e[0]:this.position=new T(...e),this.points&&this.points.position.copy(this.position)}addToScene(e=30,i=6,t=null,s=4,o=[[0,0]],r=!1){this.count=e,this.pscale.x=i*this.room.pxlEnv.pxlQuality.screenResPerc,this.isInternalTexture=!1;let v=null;r?(v=this.atlasRandomGen,o=s):v=this.atlasArrayPicker,t||(t=this.newMaterial());let d=[],h=[],u=[];for(let U=0;U<e;++U)d.push(0,0,0),h.push(Math.random(),Math.random(),Math.random()*2-1,Math.random()*2-1),u.push(...v(o));let n=new x.Float32BufferAttribute(d,3),y=new x.Float32BufferAttribute(h,4),f=new x.Float32BufferAttribute(u,2),c=new x.BufferGeometry;c.setAttribute("position",n),c.setAttribute("seeds",y),c.setAttribute("atlas",f);let m=new x.Points(c,t);return m.sortParticles=!1,m.frustumCulled=!1,this.room.scene.add(m),m.layers.set(1),m.pBaseScale=i,this.room.geoList[this.name]=m,this.geometry=c,this.material=t,this.points=m,m.position.copy(this.position),m}setUserHeight(e){this.pxlEnv.pxlCamera.userScale=e}atlasRandomGen(e=4,i=2){let t=1/e;return Array.from({length:i}).map(()=>Math.floor(Math.random()*648405.71%e)*t)}atlasRandomList(e=4,i=4,t=2){return Array.from({length:e}).map(s=>this.atlasRandomGen(i,t))}atlasArrayPicker(e){return e[Math.floor(Math.random()*92314.75%e.length)]}dupeArray(e,i){return Array.from({length:i}).fill(e)}elementDuplicator(e,i=4){return e.map(t=>this.dupeArray(t,i)).flat(1)}findLightPositions(){let e=[],i=0;return this.room.lightList.hasOwnProperty("PointLight")&&(i=this.room.lightList.PointLight.length,this.room.lightList.PointLight.forEach(t=>{e.push(t.position.clone())})),e}setAtlasPath(e){this.atlasPath=e,this.isInternalTexture=!1}useInternalAsset(e){this.atlasPath=e,this.isInternalTexture=!0}newMaterial(e=!0){let i=this.findLightPositions(),t={atlasTexture:{type:"t",value:null},noiseTexture:{type:"t",value:null},time:{type:"f",value:this.room.msRunner},pointScale:{type:"f",value:this.pscale},intensity:{type:"f",value:1},rate:{type:"f",value:.035},lightPos:{value:i}},s=this.room.pxlFile.pxlShaderBuilder(t,k(i.length),_());return s.side=x.DoubleSide,s.transparent=!0,this.isInternalTexture?s.uniforms.atlasTexture.value=this.room.pxlEnv.getAssetTexture(this.atlasPath,4,{magFilter:x.NearestFilter,minFilter:x.NearestMipmapNearestFilter}):s.uniforms.atlasTexture.value=this.room.pxlUtils.loadTexture(this.atlasPath,4,{magFilter:x.NearestFilter,minFilter:x.NearestMipmapNearestFilter}),s.uniforms.noiseTexture.value=this.room.softNoiseTexture,s.depthTest=!0,s.depthWrite=!1,e&&(this.room.textureList[this.name]=s),s}};function q(a=[8.7,8.4]){let e=g();return e+=` 
    uniform sampler2D noiseTexture;
    uniform vec2 time;
    uniform float rate;
    uniform vec2 pointScale;
    ...
    void main(){
        ...
        vec2 windDir = vec2( windInf*${a[0]}, windInf * ${a[1]} );
    ...
    void main(){
        ...
        gl_FragColor=Cd;
    }`,a}var R=class extends E{constructor(e=null,i="billowSmoke"){super(e,i),this.name=i,this.room=e}build(e=50,i=56,t=4,s=null){s||(s=[...super.dupeArray([0,0],4),...super.dupeArray([.25,0],4),...super.dupeArray([.5,0],2),...super.dupeArray([.5,.25],2),...super.dupeArray([.5,.5],2),...super.dupeArray([.5,.75],2),...super.dupeArray([.75,.75],4),...super.dupeArray([.75,.25],3),...super.dupeArray([.75,.25],3)]);let o={atlasTexture:{type:"t",value:null},noiseTexture:{type:"t",value:null},time:{type:"f",value:this.room.msRunner},pointScale:{type:"f",value:this.pscale},intensity:{type:"f",value:.8},rate:{type:"f",value:2.85}},r=this.room.pxlFile.pxlShaderBuilder(o,q(),Z());r.side=L.DoubleSide,r.transparent=!0,r.uniforms.atlasTexture.value=this.room.pxlUtils.loadTexture(this.atlasPath,4,{magFilter:L.NearestFilter,minFilter:L.NearestMipmapNearestFilter}),r.uniforms.noiseTexture.value=this.room.softNoiseTexture,r.depthTest=!0,r.depthWrite=!1,this.room.textureList[this.name]=r,super.addToScene(e,i,r,t,s)}};import*as D from"../../../Source/js/libs/three/three.module.js";function Q(a=[-.13,.15]){let e=g();return e+=`
    ...
    const vec2 windForce = vec2(${a[0]}, ${a[1]});
