...
// This is a template for a room environment
// It's a good starting point for creating a new room for pxlNav

// Include the necessary js modules
import {
  Vector2,
  Vector3,
  Color,
...
export class TemplateEnvironment extends RoomEnvironment{
  constructor( roomName='TemplateEnvironment', assetPath=null, msRunner=null, camera=null, scene=null, cloud3dTexture=null ){
    ...
    //   Or assign this.yourVariable= new Vector2(0,0);
    //
    //   Any Uniforms using Vector2, Vector3, Quaternion, etc.
    ...
    this.runTime=new Vector2(0, 0); // Curring Room Run Time; Keep as Vector2() so OpenGL shaders Auto Update when there are {x,y} changes
    this.msRunner=msRunner; // Global Timer Vector2 Object;  this.msRunner.x - Seconds Since Boot, this.msRunner.y - Average Seconds per Frame
    this.camera=camera; // pxlNav Camera Object
    this.autoCamPaths={}; // Drone Cam Paths; [{ "position", "lookAt", "up" }, {...}]
    this.scene=scene; // js Scene Object
...
// Build Scene and Assets
  ...
  build(){
    ...
    //   Prep the shader and add it to the `this.materialList` object, as the name of the Object Transform Name
    // eg. -
    //     If you'r scene object is-
    //       | RoomObject
    //       |-- RoomObjectShape
    //     Set your Material or Shader with -
    //       this.materialList[ "RoomObject" ]=*Your_Material_or_Shader*;
    ...
    /*
        let customColor = new Color( 0xff0000 ); // Hex
        customColor = new Color("rgb(255, 0, 0)"); // CSS formatted color
        let mtl=new MeshBasicMaterial({
            color:customColor
        });  
        mtl.side=FrontSide;
        this.materialList[ this.yourObjName ]=mtl;
    */
        
        
    // Assign a Lambert Material with a diffuse texture + some glow like this -
    var introTexture = new TextureLoader().load( this.assetsPath+"antibodyLobbyIntro.png" );
    introTexture.format = RGBFormat;
    ...
    /*
      ...
      this.materialList[ this.yourObjName ]=mat;
    */
