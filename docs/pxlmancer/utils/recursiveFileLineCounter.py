...
repoName="pxlmancer"
statProjectTitle="pxlmancer Code Stats"
...
#Stats file output
statPath=[ basePath+"/_show/ScriptingStats_", ".txt" ]

# Recursive Directories
dirs=[ basePath+'/js/' ]# , basePath+'/Public/' ]
avoidList=[ "libs" ]
avoidExtensions=['.py','.gif','.png','.jpg','.fbx','.webm','.webp','.bk','.ico','.properties','.settings']
...
coreCodeList=[]
coreCodeRoot="js"
coreCodeLocation=["pxlNav"] # The core code for functionality 
artistRoomList=[]
exemptList=[]
...
def lineCount(fpaths, spacer):
    ...
    while len(fpaths)>0:
        ...
        encoding = detect_encoding(curfile)

        print(curfile," - ",encoding)
        with open(curfile, encoding=encoding) as f:
            fileCount+=1
            curLineCount=0
            curComments=0
            curWhiteSpace=0
            ...
            try:
              for i, l in enumerate(f):
                ...
                print(f"Error decoding file {curfile}: {e}")
