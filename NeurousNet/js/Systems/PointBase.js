...
class Point{
  ...
  //   Velocity influence will push the point off the screen; invert velocity on given axis
    ...
    if( this.Emitter.fieldType == this.Emitter.Fields.SCREEN ){
      let padding = this.State.screenPadding;
      let maxWH = [ this.State.sW-padding, this.State.sH-padding ]
      let curPos = [ ...this.pos ]
      // Check X for bounce
      this.bounce = 0;
      if( curPos[0]<=padding || curPos[0]>=maxWH[0] ){
        curPos[0] = Math.min( Math.max(padding, curPos[0]), maxWH[0] );
      ...
      // Check Y for bounce
      if( curPos[1]<=padding || curPos[1]>=maxWH[1] ){
        curPos[1] = Math.min( Math.max(padding, curPos[1]), maxWH[1] );
      ...
      if( dist > fieldRadius*3 ){
        ...
        this.vel = multFloat( normDelta, mag( delta )*.1 );
      }
      console.log(fieldRadius, dist);
  ...
  // Limit Velocity to its Velocity Magnitude Cap
	clampVel( addWeight=0 ){
		let tmpVel = [...this.vel];
		let scaler = (tmpVel[0]**2+tmpVel[1]**2)**.5;
